...
namespace CardFool
{
    internal class Program
    {
        static int Main()
        {
            ...
            for (int i = 0; i < 100; i++)
                Results[new MTable(new MPlayer1(), new MPlayer2()).PlayGame()]++;
    ...
    public class MTable
    {
        ...
        public MTable(MPlayer1 NewPlayer1, MPlayer2 NewPlayer2,
            bool isFirstPlayerAttacking = true, List<SCard> InitDeck = null)
        {
        ...
        public void Initialize()
        {
			...
			for (int c = 0; c < MGameRules.TotalCards; c++)
            {
        ...
        public EndRound PlayDefence()
        {
            ...
            //И если не отбился от хотя бы одной карты, то вызываем ошибку
        ...
        public EndRound PlayAttack(EndRound DefenceResult)
        {
            ...
            //Атакующий не может подкинуть карты, которые не может
            //отбить обороняющийся из-за недостатка карт
            //Формально: количество небитых карт не должно превышать количество карт в руке оппонента
            if (Table.Sum(x => !x.Beaten ? 1 : 0) > GetPlayerCardCount(!IsFirstAttacking))
                throw new Exception();
        ...
        public EndGame PlayPostRound(EndRound RoundResult)
        {
            ...
            if (RoundResult == EndRound.Defend) IsFirstAttacking = !IsFirstAttacking;
